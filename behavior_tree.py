...
class RoombaBehaviorTree(BehaviorTree):
    ...
    def __init__(self):
        ...
        self.root = SelectorNode("SelectNode")
        MFN = MoveForwardNode()
        MSN = MoveInSpiralNode()
        GBN = GoBackNode()
        RN = RotateNode()
        self.root.add_child(SequenceNode("SequenceNode1"))
        self.root.add_child(SequenceNode("SequenceNode2"))
        self.root.children[0].add_child(MFN)
        self.root.children[0].add_child(MSN)
        self.root.children[1].add_child(GBN)
        self.root.children[1].add_child(RN)
